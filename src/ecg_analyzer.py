...
from ecg_phase1 import FS as DEFAULT_FS, detect_r_peaks, extract_features
...
def predict_combined(signal: np.ndarray, sr: int,
    ...
    """
    # R-peak detection
    r_peaks = detect_r_peaks(signal, sr)
    ...
    # Feature extraction
    features = extract_features(signal, r_peaks, sr)
    ...
    if final_diagnosis == "INCONCLUSIVE" and ml_result is not None:
        if ml_result["confidence"] >= CONFIDENCE_THRESHOLD:
            ...
            # Re-derive severity and recommendation from the ML-assigned label
            # by re-running the rule classifier with a nudged feature set
            # (simplest approach: reuse Phase 2 mapping)
            _sev_map = {
            ...
            sev_rec = _sev_map.get(
                final_diagnosis,
                ("LOW", "Consult a physician.")
            )
            final_severity   = sev_rec[0]
            final_recommend  = sev_rec[1]
        else:
